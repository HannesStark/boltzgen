...
than silencing that check with KMP_DUPLICATE_LIB_OK=TRUE (which can mask real
runtime issues, not just this crash), this makes sure there is only one
libomp.dylib to load in the first place.

Run once after installing boltzgen (pip install -e . or pip install boltzgen):
